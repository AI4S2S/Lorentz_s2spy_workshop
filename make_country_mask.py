...
def create_mask(da, level='Continents', path=None):
    ...
    if level not in ['Countries', 'Continents', 'US_State']:
        raise ValueError("Invalid entry for level, please use 'Countries', 'Continents' or 'US_State'")
    
    if path is None:
        folder_file = '/data/volume_2/'
    ...
    if os.path.exists(mask_file+'.nc'):
        ...
        mask_xr = xr.open_dataarray(mask_file+'.nc')
